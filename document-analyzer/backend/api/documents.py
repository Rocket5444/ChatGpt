...
def extract_text(filename: str = Query(..., min_length=1)) -> dict[str, str | int]:
    """Return extracted text verification response for a PDF file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) != ".pdf":
        raise HTTPException(status_code=400, detail="Unsupported file type for text extraction. Please provide a PDF file.")
...
def extract_tables(filename: str = Query(..., min_length=1)) -> dict[str, object]:
    """Return extracted table metadata verification response for a PDF file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) != ".pdf":
        raise HTTPException(status_code=400, detail="Unsupported file type for table extraction. Please provide a PDF file.")
...
def extract_excel(filename: str = Query(..., min_length=1)) -> dict[str, dict[str, object]]:
    """Return extracted sheet metadata verification response for an Excel file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) not in {".xlsx", ".xls"}:
        raise HTTPException(status_code=400, detail="Unsupported file type for Excel extraction. Please provide .xlsx or .xls.")
