...
def extract_text(filename: str = Query(..., min_length=1)) -> dict[str, str | int]:
    """Extract text and return preview metadata for a PDF file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) != ".pdf":
        raise HTTPException(status_code=400, detail="Unsupported file type for text extraction. Use a PDF file.")
...
def extract_tables(filename: str = Query(..., min_length=1)) -> dict[str, object]:
    """Extract table metadata from a PDF file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) != ".pdf":
        raise HTTPException(status_code=400, detail="Unsupported file type for table extraction. Use a PDF file.")
...
def extract_excel(filename: str = Query(..., min_length=1)) -> dict[str, dict[str, object]]:
    """Extract sheet metadata and preview data from an Excel file."""
    file_path = resolve_data_file(filename)
    if extract_extension(file_path.name) not in {".xlsx", ".xls"}:
        raise HTTPException(status_code=400, detail="Unsupported file type for Excel extraction. Use .xlsx or .xls.")
